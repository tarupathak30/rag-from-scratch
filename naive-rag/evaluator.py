...
EVAL_SET = [
    ...
    ("What is quantum computing?", "Quantum computing", "Quantum computing uses qubits to perform computations."),                                              # out of corpus
    ("How do neural nets learn?", "Backpropagation", "Backpropagation computes gradients via the chain rule from output to input."),                            # vague phrasing
    ("What technique prevents overfitting by randomly disabling neurons?", "Dropout", "Dropout deactivates random neurons during training to prevent overfitting."),  # paraphrased
    ("How does gradient descent use backpropagation?", "Backpropagation", "Backpropagation computes gradients via the chain rule from output to input."),       # multi-hop
]
...
def main():
    ...
    total_pt = sum(q["prompt_tokens"] for q in per_query)
    total_at = sum(q["answer_tokens"] for q in per_query)
    ...
    print(f"  Tokens — prompt: {total_pt}  answer: {total_at}  total: {total_pt + total_at}")
    ...
    report = {
        ...
        "generation": {
            ...
            "answer_tokens": total_at,
            "total_tokens":  total_pt + total_at,
        },
        "per_query": per_query,
    }
